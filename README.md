...
<a href="https://deepwiki.com/QGrain/SyzGPT"><img alt="DeepWiki" src="https://deepwiki.com/badge.svg"/></a>
...
We also generate a [wiki page](https://deepwiki.com/QGrain/SyzGPT) for SyzGPT through [DeepWiki](https://deepwiki.com/), which would auto-update weekly.
...
SyzGPT can serve as a **standalone seed generator** through SyzGPT-generator. For detailed usage, please refer to [generator/README.md](generator/README.md).
...
- A file containing target syscalls for which you want to generate seeds. We provide a `sampled_variants.txt` at [data/sampled_variants.txt](./data/sampled_variants.txt) for reproduction. For more comprehensive initial corpus generation, you can specify the [6.6_LFSC](data/6.6_LFSC) with `-f`.
...
# Pack the seeds as corpus for fuzzing
/root/fuzzers/SyzGPT-fuzzer/bin/syz-db pack WORKDIR/generated_corpus_repair_valid WORKDIR/SyzGPT_sampled_corpus.db
```

Now you can use the corpus for fuzzing by copying it to `corpus.db` under the workdir you are going to fuzz. The following step 3 is for evaluation, which users can skip.
...
SyzGPT can also cooperate with SyzGPT-fuzzer for **continuous kernel fuzzing**. For detailed usage, please refer to [generator/README.md](generator/README.md) and [fuzzer/README.md](fuzzer/README.md).
